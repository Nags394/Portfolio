...
function renderAccordion(lang) {
    ...
    if (openId) {
        const candidate = accordionRoot.querySelector(`.ac-item[data-id="${openId}"]`);
        if (candidate)
            openItem(openId);
        else
            openId = null;
...
function openItem(id) {
    ...
    const el = accordionRoot.querySelector(`.ac-item[data-id="${id}"]`);
    if (!el)
        return;
...
function renderAccordionTitle(lang) {
